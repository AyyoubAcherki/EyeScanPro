...
def charger_modele():
    """Charge le modèle avec téléchargement si nécessaire"""
    ...
    if not os.path.exists(model_path):
        try:
            ...
            url = "https://drive.google.com/uc?id=1MYgwEtP5tkGe-wLPqRFSS7oD8mKz5Vwi"
...
def preparer_image(img):
    ...
    try:
        ...
        if img.size[0] < 50 or img.size[1] < 50:
            st.warning("⚠️ Image trop petite - la qualité de la prédiction peut être réduite")
...
def page_predire_image():
    ...
    with st.expander("📸 Upload d'image", expanded=True):
        fichier = st.file_uploader("Téléchargez une image rétinienne", type=["jpg", "jpeg", "png"],
                                   help="Image claire, 224x224px minimum")
...
def page_inscription():
    st.title("📝 Enregistrement du Patient")

    with st.form("inscription"):
        col1, col2 = st.columns(2)
        prenom = col1.text_input("Prénom*", key="prenom_input")
        nom = col2.text_input("Nom*", key="nom_input")
...
def main():
    ...
    if "prenom" not in st.session_state:
        st.sidebar.warning("Veuillez d'abord vous inscrire.")
        page = "📝 Inscription"
    else:
